...
public class VentaDAO implements CrudVentaInterface<Venta, DetalleVenta>{
    ...
    public List<Venta> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                registros.add(new Venta(rs.getInt(1),rs.getInt(2),rs.getString(3),rs.getInt(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getDate(9),rs.getInt(10),rs.getInt(11),rs.getString(12)));
            }
            ps.close();
            rs.close();
    ...
    public List<DetalleVenta> listarDetalle(int id) {
        ...
        try {
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                registros.add(new DetalleVenta(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getInt(5),rs.getInt(6),rs.getInt(7),rs.getInt(8)));
            }
            ps.close();
            rs.close();
        } catch (SQLException e) {
    ...
    public boolean insertar(Venta obj) {
        ...
        try {
            ...
            ps.setInt(6, obj.getImpuesto());
            ps.setInt(7, obj.getTotal());
