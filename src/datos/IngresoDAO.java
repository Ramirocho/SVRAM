...
public class IngresoDAO implements CrudIngresoInterface<Ingreso, DetalleIngreso>{
    ...
    public List<Ingreso> listar(String texto, int totalPorPagina, int numPagina) {
        ...
        try {
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                registros.add(new Ingreso(rs.getInt(1),rs.getInt(2),rs.getString(3),rs.getInt(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getDate(9),rs.getInt(10),rs.getInt(11),rs.getString(12)));
            }
            ps.close();
            rs.close();
    ...
    public List<DetalleIngreso> listarDetalle(int id) {
        ...
        try {
            ...
            rs=ps.executeQuery();
            while(rs.next()){
                registros.add(new DetalleIngreso(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4),rs.getInt(5),rs.getInt(6)));
            }
            ps.close();
            rs.close();
        } catch (SQLException e) {
